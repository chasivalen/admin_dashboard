...
class EvaluationLibraryState(rx.State):
    ...
    # Two-step dialog control (NEW)
    readme_add_step: int = 1  # NEW - tracks which step we're on
    
    # NEW form fields for step 1
    new_eval_type: str = ""  # NEW
    new_score_type: str = ""  # NEW  
    new_pre_eval: str = ""  # NEW
    new_readme_type: str = "DEFAULT"  # NEW - for radio buttons
    new_readme_content: str = ""  # NEW - renamed from new_readme_text for clarity
    
    # NEW dialog controls for adding dropdown options
    show_eval_type_dialog: bool = False  # NEW
    show_score_type_dialog: bool = False  # NEW
    show_pre_eval_dialog: bool = False  # NEW
    new_option_input: str = ""  # NEW
    ...
    readme_view_mode: str = "list"  # "list", "add_step1", "add_step2"
    
    # Controls for inline option inputs
    show_eval_type_input: bool = False
    show_score_type_input: bool = False
    show_pre_eval_input: bool = False
    ...
    def show_add_readme_modal(self):
       ...
       # NEW setters for the new form fields
    @rx.event
    def set_new_eval_type(self, value: str):
        """NEW - Set new eval type."""
        self.new_eval_type = value
    
    @rx.event
    def set_new_score_type(self, value: str):
        """NEW - Set new score type."""
        self.new_score_type = value
    
    @rx.event
    def set_new_pre_eval(self, value: str):
        """NEW - Set new pre-eval."""
        self.new_pre_eval = value
    
    @rx.event
    def set_new_readme_type(self, value: str):
        """NEW - Set readme type (DEFAULT or CUSTOM)."""
        self.new_readme_type = value
    
    @rx.event
    def set_new_readme_content(self, value: str):
        """NEW - Set new README content."""
        self.new_readme_content = value
    
    # NEW methods for adding dropdown options
    @rx.event
    def show_add_eval_type_dialog(self):
        """NEW - Show dialog to add eval type."""
        self.show_eval_type_dialog = True
        self.new_option_input = ""
    
    @rx.event
    def show_add_score_type_dialog(self):
        """NEW - Show dialog to add score type."""
        self.show_score_type_dialog = True
        self.new_option_input = ""
    
    @rx.event
    def show_add_pre_eval_dialog(self):
        """NEW - Show dialog to add pre-eval."""
        self.show_pre_eval_dialog = True
        self.new_option_input = ""
    
    @rx.event
    def set_new_option_input(self, value: str):
        """NEW - Set input for new dropdown option."""
        self.new_option_input = value
    
    @rx.event
    def add_new_option(self, option_type: str):
        """NEW - Add new option to dropdown list."""
        if not self.new_option_input:
            return
        
        new_value = self.new_option_input.upper()
        
        if option_type == "Eval Type":
            if new_value not in self.eval_type_options:
                self.eval_type_options.append(new_value)
                self.new_eval_type = new_value
            self.show_eval_type_dialog = False
            
        elif option_type == "Score Type":
            if new_value not in self.score_type_options:
                self.score_type_options.append(new_value)
                self.new_score_type = new_value
            self.show_score_type_dialog = False
            
        elif option_type == "Pre-Eval":
            if new_value not in self.pre_eval_options:
                self.pre_eval_options.append(new_value)
                self.new_pre_eval = new_value
            self.show_pre_eval_dialog = False
...
class FilePrepState(rx.State):
    ...
    @rx.event
    def show_add_readme_form(self):
        """Show the add README form (step 1) instead of list view."""
        self.readme_view_mode = "add_step1"
        self.readme_add_step = 1
        # Reset form fields
        self.new_readme_title = ""
        self.new_eval_type = ""
        self.new_score_type = ""
        self.new_pre_eval = ""
        self.new_readme_type = "DEFAULT"
        self.new_readme_content = ""
        self.new_option_input = ""
    
    @rx.event
    def cancel_add_readme_view(self):
        """Cancel and return to list view."""
        self.readme_view_mode = "list"
        self.readme_add_step = 1
        ...
        self.new_readme_content = ""
        self.new_option_input = ""
        # Hide any option inputs
        self.show_eval_type_input = False
        self.show_score_type_input = False
        self.show_pre_eval_input = False
    
    @rx.event
    def proceed_to_step_two(self):
        """Move from step 1 to step 2 of add form."""
        if not self.new_readme_title:
            return rx.toast.error("Please enter a Read Me Title")
        self.readme_view_mode = "add_step2"
        self.readme_add_step = 2
    
    @rx.event
    def show_add_option_input(self, option_type: str):
        """Show inline input for adding new dropdown option."""
        self.show_eval_type_input = option_type == "eval_type"
        self.show_score_type_input = option_type == "score_type"
        self.show_pre_eval_input = option_type == "pre_eval"
        self.current_option_type = option_type
        self.new_option_input = ""
    
    @rx.event
    def cancel_add_option(self):
        """Cancel adding new option and hide input."""
        self.show_eval_type_input = False
        self.show_score_type_input = False
        self.show_pre_eval_input = False
        self.new_option_input = ""
    ...
    @rx.event
    def add_new_eval_type(self):
        """Add new eval type option."""
        if not self.new_option_input:
            return
        new_value = self.new_option_input.upper()
        if new_value not in self.eval_type_options:
    ...
    @rx.event
    def add_new_score_type(self):
        """Add new score type option."""
        if not self.new_option_input:
            return
        new_value = self.new_option_input.upper()
        if new_value not in self.score_type_options:
    ...
    @rx.event
    def add_new_pre_eval(self):
        """Add new pre-eval option."""
        if not self.new_option_input:
            return
        new_value = self.new_option_input.upper()
        if new_value not in self.pre_eval_options:
    ...
    @rx.event
    def save_new_readme_view(self):
        """Save the new README from view-based form."""
        if not self.new_readme_content:
            return rx.toast.error("Please enter README content")
        
        try:
            with rx.session() as session:
                from ltx_automation_app.database.models import ReadmeInstruction
                
                new_readme = ReadmeInstruction(
                    README_TITLE=self.new_readme_title.upper(),
                    README_TXT=self.new_readme_content,
                    SCORE_TYPE=self.new_score_type.upper() if self.new_score_type else None,
                    EVAL_TYPE=self.new_eval_type.upper() if self.new_eval_type else None,
                    PRE_EVAL_CONTEXT=self.new_pre_eval.upper() if self.new_pre_eval else None,
                    CONTENT_TYPE="README",
                    STATUS_IND="ACTIVE",
                    DEFAULT_IND="Y" if self.new_readme_type == "DEFAULT" else "N",
                    CUSTOM_IND="Y" if self.new_readme_type == "CUSTOM" else "N"
                )
                
                session.add(new_readme)
                session.commit()
                session.refresh(new_readme)
                new_id = str(new_readme.id)
            
            # Refresh the list
            self.load_readme_instructions()
            
            # Select the new README
            self.selected_readme_id = new_id
            self.select_readme(new_id)
            ...
            return rx.toast.success("New README Instructions saved successfully!")
            
        except Exception as e:
            return rx.toast.error(f"Error saving README: {str(e)}")
